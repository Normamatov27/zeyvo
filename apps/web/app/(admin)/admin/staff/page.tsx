      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "var(--color-fg-3)" }}>Operator / label</span>
...
function WindowRow({
  ...
  async function toggle(newStatus: string) {
    setBusy(true);
    try {
      await onStatusChange(w.id, newStatus);
    } finally {
      setBusy(false);
    }
  }
...
export default function StaffPage() {
  ...
  const totalOpen = branches.reduce(
    (s, b) => s + b.windows.filter((w) => w.status === "open").length, 0
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 15, fontWeight: 600, letterSpacing: -0.3, flex: 1 }}>
          Staff & Windows
        </span>
